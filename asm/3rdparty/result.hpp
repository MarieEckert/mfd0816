...
struct Map<Ret(void)> {
	...
	static Result<Ret, E> map(const Result<T, E> &result, Func func) {
		static_assert(
			std::is_same<T, void>::value, "Can not map a void callback on a non-void Result");
...
struct Map<Result<U, E>(void)> {
	...
	static Result<U, E> map(const Result<T, E> &result, Func func) {
		static_assert(
			std::is_same<T, void>::value, "Can not call a void-callback on a non-void Result");
...
struct Constructor<void, E> {
	static void
	move([[maybe_unused]] Storage<void, E> &&src, [[maybe_unused]] Storage<void, E> &dst, ok_tag) {}

	static void copy(
		[[maybe_unused]] const Storage<void, E> &src,
		[[maybe_unused]] Storage<void, E> &dst,
		ok_tag) {}
...
struct Result {
	...
	template <
		typename Func,
		typename Ret = Result<
			typename details::ResultOkType<typename details::result_of<Func>::type>::type,
			E>>
	...
	template <
		...
		typename Ret = Result<
			T,
			typename details::ResultErrType<typename details::result_of<Func>::type>::type>>
...
bool operator==(const Result<T, E> &lhs, const Result<T, E> &rhs) {
	static_assert(
		concepts::EqualityComparable<T>::value,
		"T must be EqualityComparable for Result to be comparable");
	static_assert(
		concepts::EqualityComparable<E>::value,
