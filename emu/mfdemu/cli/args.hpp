...
class ArgumentBase {
   public:
	 ...
	 * @param action (Optional, default false) Is it an action argument (takes no
	 * parameters).
	 */
	ArgumentBase(std::string short_name, std::string long_name = "", bool action = false);

	/**
	 * @brief Checks if the given argument and option combination works for this
	 * argument.
...
/**
 * @brief Generic child-class of ArgumentBase, this is the class which should be
 * used in combination with ArgumentParser.
 */
template <typename T>
class Argument : public ArgumentBase {
   public:
	...
	/**
	 * @brief Gets the value which was set for the argument. If it has a value and
	 * it is the first call, it will first need to convert this argument from its
	 * string form to the generic type.
